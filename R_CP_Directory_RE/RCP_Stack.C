...
void Run1_Rcp_WithStyledSystematics() {
    ...
    Double_t bin_width_half[n] = {0.35, 0.5, 0.5, 0.5, 0.5};  // half bin width per point
    ...
    // Bin width correction for first bin
    ...
    // Draw systematics boxes for 4/3 points
    TBox* systBoxes[n];
    for (int i = 0; i < n; ++i) {
        double x1 = P_t[i] - bin_width_half[i];
        double x2 = P_t[i] + bin_width_half[i];
        double y  = Rcp_4_3[i];
        double y1 = y * (1 - 0.33); // 33% down
        double y2 = y * (1 + 0.33); // 33% up
    ...
    // Draw horizontal line at R = 1.0
    TLine* line = new TLine(3.0, 1.0, 8.5, 1.0); // full x-range
    line->SetLineStyle(2);
    ...
    leg->AddEntry(g_4_3, "R_{CP} #times 4/3 (stat + syst)", "lp");
    ...
    */
    ...
    std::cout << " Saved styled Rcp plot to Run24_Rcp_StyledSystematics.root\n";
}
