...
void Energy_Calibration(TCanvas* combinedCanvas, TGraphErrors* graph, TGraphErrors* graph1) {
    ...
    graph->SetTitle("#pi^{0} #rightarrow 2#gamma Run24 Au+Au Single Particle Corrected Yields Spectra");
    graph->SetMarkerStyle(21);
    graph->SetMarkerSize(1.0);
    graph->SetMarkerColor(kBlue);
...
void Run1() {
    ...
    // Save all
    TFile *Histo_data = new TFile("Run24_H_stat_photon_prob_sp_eff_cut_sets_C.root", "RECREATE");
    //c_combined->Write();
   // c_central->Write();
    //c_peripheral->Write();
