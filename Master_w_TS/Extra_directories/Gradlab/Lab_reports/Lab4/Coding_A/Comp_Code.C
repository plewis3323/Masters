...
double PartialRatios(double N_1, double N_2, double D_1, double D_2, double d_N1, double d_N2, double d_D1, double d_D2, double& ratio_error) {
    ...
    double term1 = (1.0 / N_1) * d_N1;
    double term2 = (1.0 / N_2) * d_N2;
    double term3 = (1.0 / D_1) * d_D1;
    double term4 = (1.0 / D_2) * d_D2;
    ratio_error = R_Ratio * sqrt(pow(term1, 2) + pow(term2, 2) + pow(term3, 2) + pow(term4, 2));
...
double Composition(double R_2_1, double R_3_1, double d_R21, double d_R31, double& comp_error) {
    ...
    comp_error = Comp_R * Comp_R * sqrt(pow(d_R21, 2) + pow(d_R31, 2));
...
void main_alg() {
    ...
    // Experimental counts
    const vector<double> Counts = {4455.38, 218.274, 73.1709};
    ...
    for (size_t i = 0; i < atomic_N.size(); i++) {
        ...
        double sigma = RutherfordCrossSection(thetaDegrees, E_i, atomic_N[i], alpha_N, M_o, M_1[i], d_Mo, M1Errors[i], d_sigma_error);
        crossSections.push_back(sigma);
        crossSectionsErrors.push_back(d_sigma_error);
    }

    // Calculate partial ratios with uncertainties
    double n_ag_n_au_error, n_cu_n_au_error, n_au_n_ag_error, n_cu_n_ag_error;
    ...
    double copperComp = 1.0 - goldComp - silverComp;
    copperCompError = sqrt(pow(goldCompError, 2) + pow(silverCompError, 2));
